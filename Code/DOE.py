...
@brief This code was originally published by the following individuals for use
with Scilab:
...
def lhs(n, samples=None, criterion=None, iterations=None):
    ...
    @param samples: \e integer \n
        The number of samples to generate for each factor (Default: n). \n
    @param criterion: \e string \n
         Allowable values are "center" or "c", "maximin" or "m",
        "centermaximin" or "cm", and "correlation" or "corr". If no value
        given, the design is simply randomized. \n
    @param iterations: \e integer \n
        The number of iterations in the maximin and correlations algorithms
        (Default: 5). \n

    @return \e array:  An n-by-samples design matrix that has been normalized
...
def _lhsmaximin(n, samples, iterations, lhstype):
    ...
    @param iterations: \e integer \n
        The number of iterations in the maximin and correlations algorithms. \n
    @param lhstype: \e string \n
         Allowable values are "maximin". If "maximin" is not specified, the
         design is centered. \n
...
def _lhscorrelate(n, samples, iterations):
    ...
    for i in range(iterations):
        ...
        if np.max(np.abs(R[R != 1])) < mincorr:
            ...
            print('new candidate solution found with max,abs corrcoef = {} \
                  '.format(mincorr))
            H = cp.deepcopy(Hcandidate)
...
def _pdist(x):
    ...
    @return \e array:   A 1-by-b array of scalars, where b = m*(m - 1)/2. This
        array contains all the pair-wise point distances, arranged in the order
        (1, 0),  (2, 0), ..., (m-1, 0), (2, 1), ..., (m-1, 1), ..., (m-1, m-2).
        \n
    """

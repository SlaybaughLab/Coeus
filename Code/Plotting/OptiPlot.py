...
#            ax.errorbar(x, y, yerr=yerr, fmt=marker[i],label=label[i])   
    ...
    # Format plot
    ax.set_title('\\textbf{Average Deviation from Optimal Fitness}',fontsize=17, y=1.04)
    ax.set_ylabel('\\textbf{\% Difference from Optimal Fitness}',fontsize=15, x=-0.04) 
    ax.yaxis.set_major_formatter(majorFormatter)
    if all(y)>0:
        ax.set_yscale('log')
    ax.set_ylim(np.min(y),y[1])
    ax.set_xlabel('\\textbf{Function Evaluations}',fontsize=15, y=-0.04) 
    #ax.set_xscale('log')

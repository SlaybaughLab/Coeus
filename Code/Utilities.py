...
def Run_Transport(lst,nps=[],code='mcnp6'):
    ...
    # Execute batch
    main_jobid=sub.Popen("squeue | grep sbogetic",cwd=path,stdout=sub.PIPE,shell=True).communicate()[0].strip().split()[0]
    ...
    time.sleep(15)
    output=sub.Popen("squeue | grep sbogetic",cwd=path,stdout=sub.PIPE,shell=True).communicate()[0]

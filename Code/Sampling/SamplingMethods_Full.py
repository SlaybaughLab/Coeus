...
#---------------------------------------------------------------------------------------#
def Plot_Levy(first,xbins,second=[],sec_bins=[],alpha=1.5,gamma=1.,log=False,debug=False):  
    """
    Plots the levy distribution 
   
    Parameters
    ==========
    first : array
        The array of the Levy distribution
    xbins : array
        The bin values used to plot the disribution
   
    Optional
    ========
    second : array
        An array of a second data set to plot for comparison
    sec_bins : array
        The bin values used for the second data set
    alpha : scalar
        Levy exponent - defines the index of the distribution and controls scale properties of the stochastic process
        (Default: 1.5)
    gamma : scalar
    ...
    n, bins, patches = plt.hist(first, bins=xbins, normed=1, facecolor='grey', alpha=0.75)
    
    # Histogram of the data
    if len(second)>1:
        n, bins, patches = plt.hist(second, bins=sec_bins, normed=1, facecolor='black', alpha=0.75)
    
    # Add a 'best fit' line
    y = Levy_Function(xbins)
    l = plt.plot(xbins, y, 'r--', linewidth=1)
    
    plt.xlabel('Step Size')
    plt.ylabel('Probability')
    plt.title('Levy Flight Distribution: alpha={}, gamma={}'.format(alpha,gamma))
#    plt.axis([-20, 20, 0, 10.0])
    plt.grid(True)

    if log==True:
        plt.yscale('log', nonposy='clip')
#        plt.axis([-20, 20, 0.0001, 0.3])
